...
namespace GiantGrey.TileWorldCreator
{
    ...
    public class TilesBuildLayer : BuildLayer, ISerializationCallbackReceiver
    {
        ...
        protected TileWorldCreatorManager manager;
        ...
        protected List<int> modifiedClusters = new List<int>();
        protected HashSet<int> modifiedClustersHashSet = new HashSet<int>();
        ...
        public override void ExecuteLayer(Configuration _configuration, GameObject _owner, TileWorldCreatorManager _manager, HashSet<int> _affectedClusters = null)
        {
            ...
            if ((_affectedClusters == null || _affectedClusters.Count == 0) && (_newPositions.Count > 0 || removePositions.Count > 0))
            {
                foreach (var pos in _newPositions)
                {
                    AddClusterNeighbors(modifiedClustersHashSet, GetClusterKeyFromPosition(pos));
        ...
        private void AdjustHeightBeforeMerge()
        {
            ...
            foreach (var clusterKey in modifiedClusters)
            {
                ...
                for (int i = 0; i < cluster.transform.childCount; i++)
                {
                    ...
                    // Check on four positions to make sure to find the lowest even when there're minimal offsets.
                    for (int x = -1; x < 2; x++)
                    {
                        for (int y = -1; y < 2; y++)
                        {
                            var _offset = new Vector2((float)x * (configuration.cellSize * 0.5f), (float)y * (configuration.cellSize * 0.5f));
                            RaycastHit[] hits = Physics.RaycastAll(new Vector3(worldPos.x + _offset.x, 2000f, worldPos.z + _offset.y),
                                Vector3.down,
                                4000f, layerMask);
                    ...
                    // var _offset = new Vector2((float)(configuration.cellSize * 0.5f), (configuration.cellSize * 0.5f));
                    // RaycastHit[] hits = Physics.RaycastAll(new Vector3(worldPos.x + _offset.x, 2000f, worldPos.z + _offset.y),
                    //     Vector3.down,
                    //     4000f, layerMask);
                    //         
                    ...
                    if (targetY == Mathf.Infinity || targetY == -Mathf.Infinity)
                    {
                        targetY = 0;
                    }
                    if (found)
                    ...
                    {
                        tile.position = new Vector3(worldPos.x, worldPos.y + topOffset, worldPos.z);
                    }
                }
            }
        }
        ...
        void ProcessDualGridTiles(Vector2 worldPosition, List<TileData> newTiles, HashSet<Vector2> existingCellPositions)
        {
            for (int x = -1; x < 1; x++)
            {
                for (int y = -1; y < 1; y++)
                {
                    ...
                    // A dual grid tile at (x.5, y.5) is formed by 4 contributing blueprint cells:
                    // (x, y), (x+1, y), (x, y+1), (x+1, y+1) relative to worldPosition's base.
                    // Wait, cellPosition is already worldPosition + (-0.5 or 0.5).
                    // If cellPosition is (23.5, 6.5), it is surrounded by (23,6), (24,6), (23,7), (24,7).
                    float fx = Mathf.Floor(cellPosition.x);
                    float fy = Mathf.Floor(cellPosition.y);
                    float cx = Mathf.Ceil(cellPosition.x);
                    float cy = Mathf.Ceil(cellPosition.y);
                    
                    Vector2[] contributing = new Vector2[] {
                        new Vector2(fx, fy),
                        new Vector2(cx, fy),
                        new Vector2(fx, cy),
                        new Vector2(cx, cy)
                    ...
                    AddOrUpdateTile(cellPosition, worldPosition, newTiles, existingCellPositions, contributing);
                }
            }
        }
        ...
        {
            Vector2[] contributing = new Vector2[] { worldPosition };
            AddOrUpdateTile(worldPosition, worldPosition, newTiles, existingCellPositions, contributing);
        }

        void AddOrUpdateTile(Vector2 cellPosition, Vector2 worldPosition, List<TileData> newTiles, HashSet<Vector2> existingCellPositions, Vector2[] contributing = null)
        {
        ...
        protected virtual IEnumerator InstantiateByClusters(List<SortedTiles> _sortedTiles)
        {
#if UNITY_EDITOR
            if (!Application.isPlaying)
            {
            ...
            for (int c = 0; c < modifiedClusters.Count; c++)
            {
                ...
                if (_cluster != null)
                {
                    if (availableClusters != null)
                    {
                        var _identifier = _cluster.GetComponent<ClusterIdentifier>();
                        if (_identifier != null)
                        {
                            availableClusters.Remove(_identifier);
                        }
                    }

                    if (Application.isPlaying)
                    {
                        GameObject.Destroy(_cluster);
                    }
                    else
                    {
            ...
            for (int t = 0; t < tileLayers.Count; t++)
            {
                ...
                {
                    // Only instantiate tiles for clusters that were actually destroyed/modified
                    if (!modifiedClustersHashSet.Contains(_sortedTiles[i].clusterID)) continue;
        ...
        protected TilePreset GetRandomTilePreset(TileData _tileData, int _tileLayerIndex, ref Unity.Mathematics.Random _rng)
        {
        ...
        protected virtual void InstantiateTile(TileData _tileData, int _clusterKey, int _tileLayerIndex)
        {
        ...
        protected GameObject FindCluster(int _clusterKey, bool _createIfNotFound = true)
        {
            ...
            if (availableClusters != null)
            {
                ...
                availableClusters.RemoveWhere(x => x == null);

                foreach (var _cls in availableClusters)
                {
                    if (_cls != null && _cls.clusterID == _clusterKey)
                    {
                        _cluster = _cls;
                        break;
                    }
        ...
        public override List<TileData> GetTilesAtBlueprintCell(Vector2 _cellPosition)
        {
            List<TileData> tiles = new List<TileData>();
            float cellX = _cellPosition.x;
            float cellY = _cellPosition.y;

            if (useDualGrid)
            {
                // In dual grid, one cell (x, y) contributes to 4 tiles:
                ...
                {
                    Vector2 tilePos = new Vector2(cellX + offset.x, cellY + offset.y);
                    int hashKey = GetClusterKeyFromPosition(tilePos);
                    if (tileGrid.TryGetValue(hashKey, out var subGrid))
                    {
                        if (subGrid.TryGetValue(tilePos, out var tile))
                        {
                            tiles.Add(tile);
                        }
                    }
            ...
            {
                // Single grid: one cell (x, y) contributes to tile at (x, y)
                int hashKey = GetClusterKeyFromPosition(_cellPosition);
                if (tileGrid.TryGetValue(hashKey, out var subGrid))
                {
                    if (subGrid.TryGetValue(_cellPosition, out var tile))
                    {
                        tiles.Add(tile);
                    }
                }
        ...
        void BuildLayerUI(Configuration _asset, Editor _editor, LayerFoldoutElement _foldout, VisualElement _layerItem)
        {
            ...
            var _blueprintLayer = new PropertyField();
            _blueprintLayer.BindProperty(_layerSerializedObject.FindProperty("assignedBlueprintLayerGuid"));
            ...
            _collider.RegisterCallback<ChangeEvent<string>>(evt =>
           {
            ...
            var _placeOnTopInfo =
                new HelpBox("Places tiles on top of other tiles (Great for road tiles). Make sure that other tiles have a collision (Merging enabled).", HelpBoxMessageType.Info);
            ...
            _layerItem.Add(_placeOnTopInfo);
