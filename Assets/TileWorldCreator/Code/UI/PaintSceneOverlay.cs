...
namespace GiantGrey.TileWorldCreator
{
    ...
    public class PaintSceneOverlay : Overlay
    {
        ...
        public override void OnCreated()
        {
            ...
            var managers = Object.FindObjectsByType<TileWorldCreatorManager>(
        ...
        private static void HierarchyChanged()
        {
            ...
            // Re-build panels for all open overlays of this type
            // Note: Overlay doesn't inherit from UnityEngine.Object, so we can't use FindObjectsOfTypeAll directly easily.
            // However, we can use the SceneView.duringSceneGui or other ways to find active instances if needed.
            // For now, let's just avoid the redundant creation which was the main issue.
        }
        ...
        private static void ChangesPublished(ref ObjectChangeEventStream stream)
        {
            ...
            for (int i = 0; i < stream.length; ++i)
            {
                if (stream.GetEventType(i) == ObjectChangeKind.ChangeAssetObjectProperties)
                {
                    ...
                    var changedObj = EditorUtility.EntityIdToObject(e.instanceId);
#else
        ...
        private void BuildPanel()
        {
            ...
            managers.Clear();
#if UNITY_6000_5_OR_NEWER
            managers.AddRange(Object.FindObjectsByType<TileWorldCreatorManager>(
                findObjectsInactive: FindObjectsInactive.Include));
#elif UNITY_6000_0_OR_NEWER
            ...
            paintButton.clickable.clicked += () =>
            {
                ...
                managers[selectedManager].configuration.selectedPaintLayerGuid = "";
                managers[selectedManager].configuration.paintLayer = null;
            ...
            foreach (var folder in managers[selectedManager].configuration.blueprintLayerFolders)
            {
                ...
                foreach (var layer in folder.blueprintLayers)
                {
                    ...
                    clear.clickable.clicked += () =>
                    {
                        ...
                        managers[selectedManager].lastChangedCells = new HashSet<Vector2>(managers[selectedManager].paintedPositions);
        ...
        public void OnSceneGUI(SceneView sceneView) 
        {
            ...
            if (managers[selectedManager].configuration.paintLayer == null) { return; }
 
            Event _event = Event.current;

            if (_event.alt || _event.control || _event.command || _event.shift)
            {
                altClick = true;
            }
            else
            ...
            if (_event.type == EventType.Layout || _event.type == EventType.Repaint)
            {
                return;
            }

            if (_event.type == EventType.MouseDown || _event.type == EventType.MouseDrag || _event.type == EventType.MouseUp)
            {
                if (_event.button == 2)
                {
                    return;
            ...
            {
                if (managers[selectedManager].configuration.paintLayer == null)
                {
                    Debug.LogWarning("Please select a layer to paint");
                }
